...
class MachineController {
    ...
    async list(req, res) {
        try {
            ...
            const { status, warehouseId, model, serialNumber, fromDate, toDate } = req.query;
    ...
    async getById(req, res) {
        try {
            ...
            if (req.user.role === client_1.UserRole.WAREHOUSE_MANAGER && req.user.warehouseId !== machine.warehouseId) {
                res.status(403).json({ success: false, message: 'Forbidden' });
                return;
            }
    ...
    async update(req, res) {
        try {
            ...
            if (req.user.role === client_1.UserRole.WAREHOUSE_MANAGER && req.user.warehouseId !== existing.warehouseId) {
                res.status(403).json({ success: false, message: 'Forbidden' });
                return;
            }
